...
import pytest
import numpy as np
from drr_framework.sensitivity_tests import run_parameter_sensitivity_experiment, run_placebo_and_null_tests


def test_run_parameter_sensitivity_experiment():
    data = np.random.normal(size=(150, 2))
    res = run_parameter_sensitivity_experiment(data, window_sizes=[32, 64], methods=["welch"], tau_values=[1])
