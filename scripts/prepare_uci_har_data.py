...
class UCIHARDataProcessor:
        ...
        for window_idx in range(n_windows):
            ...
            for sample_idx in range(self.window_size):
                sample_data = {
                    ...
                    "time_seconds": (window_idx * self.window_size + sample_idx)
                    / self.sampling_rate,
    ...
    def preprocess_for_drr(self, df):
        ...
        df_clean["acc_magnitude"] = np.sqrt(
            df_clean["body_acc_x"] ** 2 + df_clean["body_acc_y"] ** 2 + df_clean["body_acc_z"] ** 2
        )
        ...
        df_clean["acc_gyro_correlation"] = df_clean["acc_magnitude"] * df_clean["gyro_magnitude"]
    ...
    def visualize_data(self, df, n_samples=2000):
        ...
        axes[1, 0].plot(
            df_viz["time_seconds"], df_viz["acc_magnitude"], label="Acc Magnitude", alpha=0.8
        )
        axes[1, 0].plot(
            df_viz["time_seconds"], df_viz["gyro_magnitude"], label="Gyro Magnitude", alpha=0.8
        )
...
def create_drr_config(data_file, config_file="config.yml"):
    ...
    config = {
        ...
        "phase_space": {"embedding_dimension": 3, "time_delay": 1, "method": "takens"},
        "visualization": {
            "save_plots": True,
            "plot_format": "png",
            "dpi": 300,
